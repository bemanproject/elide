...
concept c = std::invocable<T, Args...> && result<std::invoke_result_t<T, Args...>> && std::is_reference_v<T> &&
            (std::is_reference_v<Args> && ...);

template <typename T, typename... Args>
concept lvalue = std::invocable<T&, Args&...> && result<std::invoke_result_t<T&, Args&...>>;
...
struct elide {
    ...
    constexpr explicit elide(T t, Args... args) noexcept
        : t_(std::forward<T>(t)), args_(std::forward<Args>(args)...) {}

  private:
    static constexpr bool rvalue_noexcept_ = std::is_nothrow_invocable_v<T, Args...>;
    static constexpr bool lvalue_noexcept_ = std::is_nothrow_invocable_v<T&, Args&...>;
    using rvalue_result_type_              = std::invoke_result_t<T, Args...>;
    using lvalue_result_type_              = typename detail::elide::lvalue_result<T, Args...>::type;
