...
configure beman.emplace_from to fetch them automatically via CMake FetchContent.
...
Project-specific options are prefixed with `BEMAN_EMPLACE_FROM`.
You can see the list of available options with:

```bash
cmake -LH -S . -B build | grep "BEMAN_EMPLACE_FROM" -C 2
```
...
### `BEMAN_EMPLACE_FROM_INSTALL_CONFIG_FILE_PACKAGE`

Enable installing the CMake config file package. Default: `ON`.
Values: `{ ON, OFF }`.

This is required so that users of `beman.emplace_from` can use
`find_package(beman.emplace_from)` to locate the library.
